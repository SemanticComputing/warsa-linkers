...
class TestPersonValidation(TestCase):
    ...
    def test_get_fuzzy_current_ranks(self):
        ranks = {'promotion_date': ['"1940-02-01"^^xsd:date', '"1940-03-01"^^xsd:date', '"1940-04-06"^^xsd:date'],
                 'latest_promotion_date': ['"1940-02-01"^^xsd:date', '"1940-03-01"^^xsd:date',
                                           '"1940-04-06"^^xsd:date'],
                 'rank': ['"Sotamies"', '"Korpraali"', '"Luutnantti"']}
        person = {'properties': ranks}
        d = date(1940, 3, 5)
        ...
        ranks = {'promotion_date': ['"1940-02-01"^^xsd:date', '"1940-03-01"^^xsd:date', '"1941-03-01"^^xsd:date'],
                 'latest_promotion_date': ['"1940-05-01"^^xsd:date', '"1940-05-01"^^xsd:date',
                                           '"1941-05-01"^^xsd:date'],
                 'rank': ['"Sotamies"', '"Korpraali"', '"Luutnantti"']}
        person = {'properties': ranks}
        d = date(1943, 4, 5)
        ...
        ranks = {'promotion_date': ['"1940-02-01"^^xsd:date', '"1940-04-22"^^xsd:date', '"1940-04-22"^^xsd:date'],
                 'latest_promotion_date': ['"1940-04-21"^^xsd:date', '"1940-05-01"^^xsd:date',
                                           '"1940-05-01"^^xsd:date'],
                 'rank': ['"Sotamies"', '"Korpraali"', '"Luutnantti"']}
        person = {'properties': ranks}
        d = date(1943, 4, 5)
    ...
    def test_get_knight_score(self):
        ...
        self.assertEqual(
            self.validator.get_knight_score(person, 'ritari kenraali Karpalo', results, ctx.ranked_matches), 20)
        ...
        self.assertEqual(
            self.validator.get_knight_score(person, 'ritari kenraali Karpalo', results, ctx.ranked_matches), 20)
        self.assertEqual(self.validator.get_knight_score(other, 'ritari kenraali Karpalo', results, ctx.ranked_matches),
                         -20)
        ...
        self.assertEqual(self.validator.get_knight_score(person2, 'ritari A. Snellman', results, ctx.ranked_matches),
                         -20)
    ...
    def test_preprocessor(self):
        ...
        self.assertEqual(preprocessor("Kenraali Karl Oesch seuraa maastoammuntaa."),
                         'kenraalikunta Karl Oesch seuraa maastoammuntaa.')
        self.assertEqual(preprocessor("Korkeaa upseeristoa maastoammunnan Aunuksen kannaksen mestaruuskilpailuissa."),
        ...
        self.assertEqual(preprocessor("Eräs Brewster-koneista, jotka seurasivat marsalkan seuruetta."),
                         'Eräs Brewster-koneista, jotka seurasivat # kenraalikunta Mannerheim # seuruetta.')
        self.assertEqual(preprocessor("Kenraali Walden Marsalkan junassa aterialla."),
        ...
        self.assertEqual(preprocessor(
            "Vas: eversti Laaksonen, kapteeni Karu, ylikersantti Vorho, ja alikersantit Paajanen ja Nordin filmattavina. Oik. komentajakapteeni Arho juttelee muiden Mannerheim-ritarien kanssa."),
                         'Vas: eversti Laaksonen, kapteeni Karu, ylikersantti Vorho, ja alikersantti Paajanen alikersantti Nordin filmattavina. Oik. komentajakapteeni Arho juttelee muiden Mannerheim-ritarien kanssa.')
        self.assertEqual(preprocessor(
            "Majuri Laaksosen komentopaikka mistä johdettiin viivytystaistelua Karhumäkilinjalla. Majuri Laaksonen seisomassa kuvan keskellä."),
                         'Majuri Laaksosen komentopaikka mistä johdettiin viivytystaistelua Karhumäkilinjalla. Majuri Laaksonen seisomassa kuvan keskellä.')
        self.assertEqual(preprocessor("Luutn. Juutilainen Saharan kauhu jouluk. Alussa."),
                         'luutnantti Juutilainen # Aarne Edward Juutilainen # jouluk. Alussa.')
        self.assertEqual(preprocessor("Kapteenit Palolampi ja Juutilainen ratsailla Levinassa."),
        ...
        self.assertEqual(preprocessor("kenraalit keskustelevat pienen tauon aikana, vas: eversti Paasonen, "
                         ...
                         'kenraalimajuri Palojärvi, kenraalimajuri Svanström, Yl.Esikuntapäällikkö jalkaväenkenraali Heinrichs ja eversti Vaala.')
        self.assertEqual(preprocessor(
            "Radioryhmän toimintaa: Selostaja työssään ( Vänrikki Seiva, sot.virk. Kumminen ja Westerlund)."),
        ...
        self.assertEqual(preprocessor(
            "Ulkomaisten lehtimiesten retkikunta etulinjan komentopaikalla Tornion rintamalla 3/10-44. Komentaja, everstiluutnantti Halsti selostaa tilannetta kaistallaan piirtäen kepillä kartan maantiehen. Komentajasta oikealla: Björnsson Mehlem, sot.virk.Zenker, Farr, luutnantti Miettinen,etualalla oikealla Scott."),
        ...
        self.assertEqual(preprocessor(
            "Viestiosasto 1: Sotilasradiosähköttäjien tutkinossa 27.4.1942 todistuksen saaneet, vas. oikealle: Vänrikki Aro, korpraali Räsänen, vänrikki Nordberg, sotilasmestari Kivi, luutnantti Päiviö, sotilasmestari Lavola, sot.virk. Halonen, alikersantti Rosenberg, vänrikki Lindblad, sot.virk. Österman, alikersantti Salenius."),
        ...
        self.assertEqual(preprocessor("Ev. luutn.Pasonen ja saks. Amiraali keskuselevat"),
        ...
        self.assertEqual(preprocessor("TK-Pärttyli Virkki erään lennon jälkeen."),
        ...
        self.assertEqual(preprocessor("osaston komentajalle majuri Murolelle joukkojensa"),
                         'osaston komentajalle majuri Murole joukkojensa')
    ...
    def test_pruner(self):
        ...
        self.assertEqual(pruner('Höpö höpö Engelbrecht'), 'Höpö höpö Engelbrecht')
        self.assertEqual(pruner('Everstiluutnantti Berndt Eino Edvard Polón'),
    ...
    def test_replace_sv_list(self):
        self.assertEqual(persons.replace_sv_list(
            "TK-rintamakirjeenvaihtaja Yläjärvellä (vas. Sot.virk. Kapra, Jalkanen, vänr. Rahikainen)."),
                         'TK-rintamakirjeenvaihtaja Yläjärvellä (vas.  sotilasvirkamies Kapra, sotilasvirkamies Jalkanen,vänr. Rahikainen).')
        self.assertEqual(persons.replace_sv_list("Sotilasvirk. Kapra, Jalkanen, vänr. Rahikainen)."),
                         ' sotilasvirkamies Kapra, sotilasvirkamies Jalkanen,vänr. Rahikainen).')
        self.assertEqual(persons.replace_sv_list("Sotilasvirkailija Kapra, Jalkanen, vänr. Rahikainen)."),
                         ' sotilasvirkamies Kapra, sotilasvirkamies Jalkanen,vänr. Rahikainen).')
        self.assertEqual(persons.replace_sv_list(
            "Komentajasta oikealla: Björnsson Mehlem, sot.virk.Zenker, Farr, luutnantti Miettinen,etualalla oikealla Scott."),
